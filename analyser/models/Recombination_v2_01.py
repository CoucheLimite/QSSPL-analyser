...
class Constants(object):
    ...
    Width = 0.018
    Eg = 1.12   #eV
    vth = 1.1e7 #cm/s Thermal velocity from W. M. Bullis and H. R. Huff, J. Electrochem. Soc. 143, 1399 ͑1996͒.
...
class SRH(Constants):
    ...
    def Iron(self,N):
        #Taken from Iron detection in crystalline silicon by carrier lifetime measurements for arbitrary injection and doping 2004
    ...
    def SchroderTextbook_SRH(self,tau_n,tau_p,Et=0):
        ...
        # It is assumed that Nt << p, n, and that the semiconductor is not degenerate.
    ...
    def PlotAll(self,plt):
        ...
        ax.plot(self.Deltan,self.SchroderTextbook_SRH(self.tau_n,self.tau_p,self.Et),'-.',label='Schroder Textbook')
        legend(loc=0)
        loglog()
...
class Emitter_Recombination(Constants):
    ...
    def PlotAll(self,plt):
        ...
        ax.plot(self.Deltan,self.tau_EmitterRecombination(),':',label='Emitter Recombination:'+str(self.J0e))
...
class Auger(Constants):
    ...
    def Richter2012(self):
        ...
        (K_n*g_eeh*n_0+K_p*g_ehh*p_0+K_Delta*self.Deltan**delta)
...
class Radiative(Constants):
    ...
    def Altermatt2005(self):    
        #The version appearing in xxx, belive this one is correct. Though b1 and B3 may need to divded by 2.
...
class TotalLifetime(Auger,SRH,Emitter_Recombination,Radiative):
    ...
    def PlotiComponants(self,plt=False):
        if plt==False:
            plot(self.Deltan,self.itau_SRH(),'--'
                ,label='SRH')
            plot(self.Deltan,self.itau_aug(),'--'
                ,label='Auger')
            plot(self.Deltan,self.itau_rad(),'--'
                ,label='Radiative')
            plot(self.Deltan,self.itau_EmitterRecombination(),'--'
                ,label='Emitter')
        else:
            plt.plot(self.Deltan,self.itau_SRH(),'--'
                ,label='SRH')
            plt.plot(self.Deltan,self.itau_aug(),'--'
                ,label='Auger')
            plt.plot(self.Deltan,self.itau_rad(),'--'
                ,label='Radiative')
            plt.plot(self.Deltan,self.itau_EmitterRecombination(),'--'
                ,label='Emitter')           

    def PlotComponants(self,plt=False):
        if plt==False:
            plot(self.Deltan,self.tau_SRH(),'--'
                ,label='SRH')
            plot(self.Deltan,self.tau_aug(),'--'
                ,label='Auger')
            plot(self.Deltan,self.tau_rad(),'--'
                ,label='Radiative')
            plot(self.Deltan,self.tau_EmitterRecombination(),'--'
                ,label='Emitter')
        else:
            plt.plot(self.Deltan,self.tau_SRH(),'--'
                ,label='SRH')
            plt.plot(self.Deltan,self.tau_aug(),'--'
                ,label='Auger')
            plt.plot(self.Deltan,self.tau_rad(),'--'
                ,label='Radiative')
            plt.plot(self.Deltan,self.tau_EmitterRecombination(),'--'
                ,label='Emitter')
...
#        Constants.__init__(self)       #not needed?, as inherrants properties from SHR,Ayger and Radiative it gets the Constants Class
        SRH.__init__(self)
        Auger.__init__(self)
...
#        tau_SRH = 1/itau_SRH    #tau_SRH, ambiguous name, irection
        ...
        #limits fit to exclude Et, why is this done? why not just reference tau_SRH?
        f1 = lambda Deltan, tau_n, tau_p: self.tau_SRH(Deltan,abs(tau_n),abs(tau_p))
        f2 = lambda Deltan, tau_n, tau_p: self.itau_SRH(Deltan,abs(tau_n),abs(tau_p))

        tau_SHR = (1/self.tau_m - self.itau_aug()-self.itau_rad())**(-1)
        itau_SHR = 1/self.tau_m - self.itau_aug()-self.itau_rad()

        popt, pcov = scipy.optimize.curve_fit(f1, self.Deltan, tau_SHR,p0=[1e-3,1e-3])
        ipopt, ipcov = scipy.optimize.curve_fit(f2, self.Deltan, itau_SHR,p0=[1e-3,1e-3])
...
#        ax1.plot(self.Deltan,self.tau_SRH(Deltan),'-',label='tau srh')
        ax1.plot(self.Deltan,1/(self.itau_SRH(self.Deltan,*popt)+self.itau_aug()+self.itau_rad()),'--',label='Fitted')

        ax2.plot(self.Deltan,1/self.tau_m/1e3,label='Original')
        ax2.plot(self.Deltan,(self.itau_SRH(self.Deltan,*ipopt)+self.itau_aug()+self.itau_rad())/1e3,'--',label='Fitted')
...
def CheckFittingJ0():
    ...
    text(1e12, 1e-1, 'Doping: '+str(Constants.Doping)+' cm^-3', ha='center', va='center')
    text(1e12, 0.7e-1, 'Width: '+str(Constants.Width)+' cm', ha='center', va='center')
    text(1e12, 0.4e-1, 'J0e: '+str(Emitter_Recombination().J0e)+' Acm^-2', ha='center', va='center')
